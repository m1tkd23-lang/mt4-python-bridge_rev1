...
* related_files:
  ...
  * src\gui_common\widgets\chart_widget.py（TASK-0141 移設先 / TASK-0142 で canonical 化完了。暫定的に `backtest_gui_app.styles.DARK_THEME_COLORS` / `style_matplotlib_figure` を import する逆依存を内包しており、T-E で `gui_common.styles` 新設時に解消予定）
  * src\gui_common\widgets\time_series_chart_widget.py（TASK-0141 移設先 / TASK-0142 で canonical 化完了。スタイル import は `chart_widget.py` と同条件）
  * src\backtest_gui_app\constants.py
