...
* related_files:
  ...
  * src\gui_common\widgets\collapsible_section.py（TASK-0141 移設先 / TASK-0142 で canonical 化完了、`CollapsibleSection`）
  * src\gui_common\widgets\chart_widget.py（TASK-0141 移設先 / TASK-0142 で canonical 化完了、`MatplotlibChart`）
  * src\gui_common\widgets\time_series_chart_widget.py（TASK-0141 移設先 / TASK-0142 で canonical 化完了、`TimeSeriesChartWidget`）
...
* task_split_notes:
  ...
  * [TASK-0141 実装済み / 2026-04-18] T-D 第1段（§12-2 2段移設の第1段）として `CollapsibleSection` / `MatplotlibChart` / `TimeSeriesChartWidget` の3ウィジェットを `src\backtest_gui_app\widgets\` から `src\gui_common\widgets\` へ物理移設。第1段時点では `src\backtest_gui_app\widgets\{collapsible_section,chart_widget,time_series_chart_widget}.py` を旧 import パス互換の再エクスポートシムとして残置した（当時は後続 F6 で削除予定。F6 = TASK-0142 で 2026-04-18 に完全撤去済み、次エントリ参照）。`SummaryPanel` / `InputPanel` / `ResultTabs` / `AllMonthsTab` / `ChartOverviewTab` / `BacktestResultPresenter` の import 文は第1段時点ではシム経由で動作していた（正式 import は `gui_common.widgets.*`）。また T-D / S-3 として `SummaryPanel` の MR 11 項目表示を新規共通ウィジェット `gui_common.widgets.mean_reversion_summary_widget.MeanReversionSummaryWidget` へ切り出し、`SummaryPanel.mean_reversion_widget` として露出。`BacktestResultPresenter._populate_mean_reversion` / `clear_result_views` は共通ウィジェットの `set_summary(MeanReversionSummary | None)` を呼ぶだけの薄い委譲に縮約。`summary_labels` 辞書から MR キー（`mr_*` 11 個）は除去され、MR 表示の状態管理は共通ウィジェット側に集約
  * [TASK-0142 実装済み / 2026-04-18] T-D 第2段（§12-2 2段移設の第2段）として `src\backtest_gui_app\widgets\{collapsible_section,chart_widget,time_series_chart_widget}.py` の再エクスポートシム3点を完全削除し、`all_months_tab.py` / `chart_overview_tab.py` / `input_panel.py` / `result_tabs.py` / `explore_gui_app\views\input_panel.py` / `TEST\task0120_summary_panel_visual_check.py` の旧 import 6 箇所を canonical 経路 `gui_common.widgets.*` へ書き換え。repo 全体 grep で `backtest_gui_app.widgets.(collapsible_section|chart_widget|time_series_chart_widget)` の残存ゼロを確認済み（`bac/` 配下のバックアップ資材は実行対象外）。再エクスポートシム運用は T-D 完了を以て終了した
...
* related_files:
  ...
  * src\backtest_gui_app\styles\dark_theme.py（`DARK_THEME_COLORS` / `style_matplotlib_figure` は `gui_common.styles` から re-export。`DARK_THEME_QSS` / `apply_dark_theme` は backtest_gui_app 固有のまま残置。TASK-0146 で explore_gui 側からも `apply_dark_theme` を直接 import する利用元となる）
  * src\explore_gui_app\views\main_window.py（TASK-0146 で `ExploreMainWindow.__init__` 末尾に `apply_dark_theme(self)` 適用を追加し、backtest_gui と同一 QSS を適用）
  * src\gui_common\styles\__init__.py（TASK-0145 新設 / T-E 第1段で canonical 化）
  * src\gui_common\styles\dark_theme.py（TASK-0145 新設 / `DARK_THEME_COLORS` と `style_matplotlib_figure` の canonical 定義。gui_common.widgets 参照部のみ抽出・backtest_gui_app 固有 QSS/apply は非対象）
  * src\backtest_gui_app\views\main_window.py
  * src\backtest_gui_app\views\input_panel.py
  * src\backtest_gui_app\views\result_tabs.py
  * src\backtest_gui_app\views\summary_panel.py
  * src\gui_common\widgets\collapsible_section.py（TASK-0141 移設先 / TASK-0142 で canonical 化完了）
  * src\gui_common\widgets\chart_widget.py（TASK-0141 移設先 / TASK-0142 で canonical 化完了。TASK-0145 で `backtest_gui_app.styles` 逆依存を解消し、canonical 経路 `gui_common.styles.{DARK_THEME_COLORS, style_matplotlib_figure}` へ切替済み）
  * src\gui_common\widgets\time_series_chart_widget.py（TASK-0141 移設先 / TASK-0142 で canonical 化完了。スタイル import は TASK-0145 で `chart_widget.py` と同経路 `gui_common.styles` へ canonical 化済み）
  * src\backtest_gui_app\constants.py
...
* task_split_notes:
  ...
  * [TASK-0141 実装済み / 2026-04-18] T-D 第1段で `MatplotlibChart` / `TimeSeriesChartWidget` / `CollapsibleSection` の定義を `gui_common.widgets` 配下に移設（旧 `backtest_gui_app.widgets.*` は一旦再エクスポートシムとして残置、TASK-0142 で完全撤去済み・次エントリ参照）。移設後も Figure facecolor・軸色・grid 色のダーク統一挙動は `style_matplotlib_figure` 経由で維持。`gui_common.widgets.chart_widget` / `time_series_chart_widget` は暫定的に `backtest_gui_app.styles` から import する逆依存を内包しており、T-E で `gui_common.styles` 新設時に解消予定
  * [TASK-0142 実装済み / 2026-04-18] T-D 第2段で `backtest_gui_app.widgets.{collapsible_section,chart_widget,time_series_chart_widget}` の再エクスポートシム3点を完全削除し、旧 import 経路を利用していた 6 箇所（`all_months_tab.py` / `chart_overview_tab.py` / `input_panel.py` / `result_tabs.py` / `explore_gui_app\views\input_panel.py` / `TEST\task0120_summary_panel_visual_check.py`）を canonical 経路 `gui_common.widgets.*` へ書き換え。ダーク統一の適用経路は変わらず、ウィジェット側の canonical 化のみが進んだ状態
  * [TASK-0145 実装済み / 2026-04-18] T-E 第1段で `src/gui_common/styles/`（`__init__.py` + `dark_theme.py`）を新設し、`DARK_THEME_COLORS`（17 色トークン）と `style_matplotlib_figure` を canonical 定義として移設。`backtest_gui_app/styles/dark_theme.py` は両者を `gui_common.styles` から import する形に書き換え、`DARK_THEME_QSS` / `apply_dark_theme` は backtest_gui_app 固有スタイル（QSS + QApplication/QWidget 適用）として残置。`gui_common.widgets.{chart_widget,time_series_chart_widget}` が内包していた `backtest_gui_app.styles` 逆依存（層破り）は本 task で完全解消され、canonical 経路は `gui_common.styles.{DARK_THEME_COLORS, style_matplotlib_figure}` に一本化。TASK-0143 の F6 シム削除ポリシーに倣い、第1段 move + 全 import 書き換えを同一 task 内で完結（シム残置なし）
  * [TASK-0146 実装済み / 2026-04-18] T-E 第2段で `explore_gui_app/views/main_window.py` の `ExploreMainWindow.__init__` 末尾に `apply_dark_theme(self)` 適用を追加し、`backtest_gui_app.styles.dark_theme` から `apply_dark_theme` を直接 import（canonical 経路確定は後続 task で判断）。これにより explore_gui MainWindow 起動時に `DARK_THEME_QSS`（backtest_gui と同一定義）が適用され、§11-2 T-E の本来の完了条件『explore_gui の見た目が backtest_gui と統一される』を達成。offscreen smoke で `ExploreMainWindow().styleSheet()` が `DARK_THEME_QSS` と一致することを確認済み
