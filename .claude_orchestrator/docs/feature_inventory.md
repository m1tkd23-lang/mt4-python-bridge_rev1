...
* layer: gui
* status: partial (戦略選択肢追加済、GUI 実起動での動作未検証)
...
* task_split_notes:
  * 2026-04-21 v2: `_AVAILABLE_STRATEGIES` に `bollinger_range_A`, `bollinger_combo_AB` 追加
  * `BOLLINGER_PARAM_VARIATION_RANGES` に `bollinger_range_A` エントリ追加(v4_4 と同範囲を流用)
  * コード上は `run_backtest` / `run_bollinger_exploration` 両経路で A/B/combo_AB が走ることを確認済 (subprocess レベル)
  * **未確認**: GUI 実起動(QApplication 立ち上げ) による対策①〜④/combo_AB 選択・実行
  * 承認ゲートは未接続

* notes:
  * GUI 実行未検証のため UI フリーズや widget 不整合の可能性あり、実起動確認が必要
