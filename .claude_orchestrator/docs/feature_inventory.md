...
* task_split_notes:
  ...
  * 探索ループ（exploration_loop.py）と evaluate_cross_month() / evaluate_integrated() の接続は未実装。現状は CLI / GUI から手動でパラメータ変更→評価のワークフロー

* notes:
  * 基準判定ロジック（evaluate_cross_month / evaluate_integrated）は実装済みだが、探索ループとの自動接続が未実装のため partial
