...
- 実行日時: 2026-04-10 22:23
- task_type: chore
- risk_level: low

### 変更内容
bollinger_range_v4_4_tuned_a.py は既にコミット bfdcf32 (TASK0098) で正式追跡されており、untracked 状態は解消済み。ただし GUI (_AVAILABLE_STRATEGIES) ...

### 関連ファイル
- none

### 注意点
- tuned_a はコミット済みだがコード上どこからも参照されていない。exploration が追加バリアントを生成した場合に同様の管理課題が再発する可能性がある。


## TASK-0099 : explore_gui Phase 2 テーブル高さの候補数に応じた動的調整（UX 改善）

- 実行日時: 2026-04-10 22:32
- task_type: feature
- risk_level: low

### 変更内容
Phase 2 テーブル高さを候補数に応じて動的調整し、ログパネル最小高さ 120px を設定して過圧縮を防止した。

### 関連ファイル
- src/explore_gui_app/views/result_panel.py

### 注意点
- ROW_HEIGHT=30px は推定値のため、高DPIやカスタムスタイル環境で行高が異なる場合は微調整が必要になる可能性がある
- 手動目視確認（Phase 1→2 フロー・小画面）が未実施のため、実機で想定外のレイアウト崩れが発覚する可能性は残る