...
* GUI 経由の探索統合（将来スコープ）
* 複数月評価フロー導入（CSV選択モード対応）: input_panel に Selected 3 months / All CSVs / Custom の3モードを追加し、探索フローを複数月安定性中心に転換する（詳細は `複数月評価フロー方針.md` 参照）
* ~~BollingerLoopConfig / BollingerExplorationConfig に csv_paths: list[str] | None フィールドを追加し、csv_paths > csv_dir > csv_path の優先順位で横断評価ロジックを実装する（優先順位ロジックは TASK-0090 で確定済み。詳細は複数月評価フロー方針.md「csv_path / csv_paths / csv_dir フィールド定義と優先順位」セクション参照）~~ → TASK-0091 で ExplorationConfig・BollingerExplorationConfig・BollingerLoopConfig・LoopConfig の4 dataclass に csv_paths フィールド追加、_resolve_csv_files ヘルパーによる優先分岐ロジック実装完了（2026-04-10）
* refine の複数月集約ベース化: 単月結果ではなく選択された複数月の集約評価結果（integrated_evaluation）をベースにスコアリングする方式へ移行する（現行 refinement.py のスコアリング構造は概ね対応済み）
