...
## 2026-04-22 v5 追加 — B 戦術の役割と開発方針の確定

### 重要な方針確定 (原則)

**B 戦術は A 戦術の補完専用**。A が主戦術であり、年間プラス収支の本線を担う。

### 判断基準

- **B 単独 +100 pips/年 程度ではリスクリターン的に微妙**、本線にしない
- B の評価は **combo_AB で A+B が A 単独を上回るか** が基準(崩壊月ゼロ維持含む)
- B は A が発動しない/負ける相場(明確な一方向 trend 等)でのみ発火すべき
- **A が入れる局面に B が逆方向で相殺する設計は禁則**
- 単独 BT の成績だけで B を拡張しない

### 参考: 2024 OOS の実測(Dukascopy)

| 戦術 | 2024 OOS total | crash | 備考 |
|---|---:|---:|---|
| A 単独 | +374.1 | 1 | 2024-07 (-113) のみ崩壊 |
| B2 v3 単独 | +74.9 | 5 | 独立 BT では微妙 |
| B2 v5 単独 (peak_trail T30) | +114.7 | 4 | 独立 BT では微妙 |
| B3 v8 単独 | -18 | 5 | BB 拡大型、保留 |
| 旧 B 単独 | -437 | 5 | 参考 |
| combo_AB (旧B) | -366 | 7 | A の +374 を B が食い潰している |

**示唆**: 現状の B 設計は combo_AB で A の成績を食い潰しており、役割原則を満たしていない。B の再設計が必要(A が負けた月だけ B がプラスを出す相補関係)。

### 次の B 開発で守ること

1. **combo_AB BT を常に並行実行**: A 単独 vs A+B の差分で判断
2. **A の崩壊月(2024-07 等)で B がプラスを出せるか**を評価軸に
3. A が機能している月は B が発動しない設計(H1 明確トレンド時のみ等)
4. **A の取引には触れない**(B は別 magic、lane 完全分離)
