...
1. 共有ウィジェット・スタイルの中立化（widgets は T-D = TASK-0141/0142 で `gui_common/widgets/` へ canonical 化完了。styles は T-E 着手まで `backtest_gui_app/styles/` に残置）  
   ...
   * styles: T-E 第1段（= TASK-0145）で `src/gui_common/styles/` を新設し、`DARK_THEME_COLORS` と `style_matplotlib_figure` を canonical 定義として移設・`gui_common.widgets.{chart_widget,time_series_chart_widget}` が内包していた `backtest_gui_app.styles` 逆依存（層破り）を解消済み。`backtest_gui_app/styles/dark_theme.py` は両者を `gui_common.styles` から import する形に書き換え、`DARK_THEME_QSS` / `apply_dark_theme` のみ backtest_gui_app 固有として残置。explore_gui 側 `MainWindow.__init__` への `apply_dark_theme(self)` 適用（見た目統一の第2段）は T-E 残スコープとして別 task で起案する。
...
* T-D: 共通ウィジェット（`CollapsibleSection` / `MatplotlibChart` / `TimeSeriesChartWidget`）の `gui_common` 化判断  
  ...
  * **[TASK-0145 実装済み / 2026-04-18]** T-D 完了時点で `gui_common.widgets.{chart_widget,time_series_chart_widget}` に残存していた `backtest_gui_app.styles` 逆依存（層破り）を T-E 第1段で解消。`src/gui_common/styles/`（`__init__.py` + `dark_theme.py`）を新設し、`DARK_THEME_COLORS` と `style_matplotlib_figure` を canonical 定義として移設、`backtest_gui_app/styles/dark_theme.py` は両者を `gui_common.styles` から import する形に書き換え（`DARK_THEME_QSS` / `apply_dark_theme` は backtest_gui_app 固有として残置）。TASK-0143 の F6 シム削除ポリシーに倣い、第1段 move + 全 import 書き換えを同一 task 内で完結し、シムは残置していない。T-D で積み残した styles 層の依存方向逆転は本 task で完全解消済み。T-E 残スコープ（explore_gui 側 `MainWindow.__init__` への `apply_dark_theme(self)` 適用による見た目統一）は §11-2 T-E で別 task として起案する。
* T-E: ダークテーマの explore_gui 適用  
  ...
  * **[TASK-0145 実装済み / 第1段完了 / 2026-04-18]** `src/gui_common/styles/` を新設して `DARK_THEME_COLORS` / `style_matplotlib_figure` を canonical 化し、`gui_common.widgets.{chart_widget,time_series_chart_widget}` の `backtest_gui_app.styles` 逆依存（層破り）を解消。`DARK_THEME_QSS` / `apply_dark_theme` は backtest_gui_app 固有スタイルとして `backtest_gui_app/styles/dark_theme.py` に残置。
  * **[TASK-0146 実装済み / 第2段完了 / 2026-04-18]** `explore_gui_app/views/main_window.py` の `ExploreMainWindow.__init__` 末尾に `apply_dark_theme(self)` 適用を追加（`backtest_gui_app.styles.dark_theme.apply_dark_theme` を直接 import）。これで本 T-E の完了条件『explore_gui の見た目が backtest_gui と統一される』を達成。offscreen smoke で `ExploreMainWindow().styleSheet()` が `DARK_THEME_QSS` と一致することを確認済み。`apply_dark_theme` / `DARK_THEME_QSS` の canonical 化範囲（`gui_common.styles` へ移すか backtest_gui_app 固有で残すか）は本 task 対象外で据え置き、後続 task で判断予定。
