...
* チャート widget 群: `backtest_gui_app/widgets/`（chart / price_chart / time_series / linked_trade / collapsible_section）
...
* CollapsibleSection / MatplotlibChart / TimeSeriesChartWidget  
  `backtest_gui_app/widgets/` → explore_gui 側でも同じウィジェットが必要になる見込み。
* ダークテーマ適用基盤  
...
1. 共有ウィジェット・スタイルの中立化（Phase 1 では現配置維持で確定）  
   * `backtest_gui_app/widgets/`, `backtest_gui_app/styles/` は **Phase 1 ではそのまま `backtest_gui_app` 配下に据え置く**。explore_gui_app からの現行参照は `backtest_gui_app.widgets.collapsible_section` 1 箇所のみで、GUI 同士の相互参照として許容する（backend → GUI パッケージ依存のような層破りは発生していない）。
   * 新規で explore_gui 側に widget / style を追加する際も、Phase 1 のうちは `backtest_gui_app.widgets.*` / `backtest_gui_app.styles.*` を直接 import してよい。
   * Phase 2 で中央回帰分析サマリー表示や All Months タブ相当を explore_gui に移す実装タスクで「gui_common 化が必要かどうか」を改めて評価する（必要になった時点で段階的に `src/gui_common/widgets/`, `src/gui_common/styles/` へ移す）。
...
* タブ C「Analysis」: Phase 2 結果に対して `analyze_all_months_mean_reversion` を適用し、MR サマリー・全期間集約・月別ばらつきを大きく表示する読み取り専用ビュー。Compare A/B 相当もここに統合候補（実装は Phase 2 後半）。
...
* `gui_common/widgets/` 移設（T-D）は他に影響範囲が広い。Phase 1 Step 2（`gui_common/strategy_params.py`）と同様、move + 再エクスポートシム残置 → 後続タスクでシム削除の 2 段で進める。
