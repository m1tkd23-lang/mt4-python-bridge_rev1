...
int BridgeCountOpenPositions(const BridgeSettings &settings)
{
    int count = 0;
    ...
    for(int i = OrdersTotal() - 1; i >= 0; i--)
    {
        ...
        if(OrderMagicNumber() != settings.magic_number)
        {
            continue;
        }
...
)
{
    ...
    string comment = "cmd:" + StringSubstr(command_id, 0, 24);
...
void BridgeProcessNextCommand(const BridgeSettings &settings)
{
    ...
    if(BridgeCountOpenPositions(settings) > 0)
    {
        BridgeWriteResult(settings, command_id, "rejected", action, -1, 0, "position already exists");
        BridgeDeleteFile(settings, relative_path);
        return;
    }

    int new_ticket = BridgeExecuteMarketOrder(settings, action, command_id, sl, tp);
    if(new_ticket < 0)
    {
