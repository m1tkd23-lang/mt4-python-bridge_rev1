...
def _base_close_compare_signal(
    ...
    if len(bars) < required_bars():
        raise SignalEngineError(
            f"At least {required_bars()} bars are required to evaluate close_compare_v1"
        )
    ...
    if latest_bar.close > previous_bar.close:
        return (
            SignalAction.BUY,
            f"latest close {latest_bar.close} is greater than previous close {previous_bar.close}",
        )

    if latest_bar.close < previous_bar.close:
        return (
            SignalAction.SELL,
            f"latest close {latest_bar.close} is lower than previous close {previous_bar.close}",
        )

    return (
        SignalAction.HOLD,
        f"latest close {latest_bar.close} is equal to previous close {previous_bar.close}",
    )


def evaluate_close_compare_v1(
    market_snapshot: MarketSnapshot,
    ...
    base_action, base_reason = _base_close_compare_signal(market_snapshot)
