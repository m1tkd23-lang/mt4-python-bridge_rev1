# src/mt4_bridge/strategies/bollinger_range_v4_4_1.py
...
# =========================
# v4.4.1 追加: range強化ガード
# =========================
# 緩やかな片方向相場を range と誤認しないための追加条件
...
) -> tuple[bool, str, float, float, float, bool]:
...
def _determine_market_state(
    *,
    ...
    (
        ...
        range_reason,
        distance_from_middle,
        _recent_middle_proximity_ratio,
        _recent_above_ratio,
        _recent_has_both_sides,
    ) = _is_range(
        closes=closes,
...
def _base_signal(
    ...
    if len(bars) < required_bars():
        raise SignalEngineError(
            f"At least {required_bars()} bars are required to evaluate bollinger_range_v4_4_1"
        )
...
def evaluate_bollinger_range_v4_4_1(
    market_snapshot: MarketSnapshot,
    position_snapshot: PositionSnapshot,
    strategy_name: str = "bollinger_range_v4_4_1",
) -> SignalDecision:
    bars = market_snapshot.bars
    if len(bars) < required_bars():
        raise SignalEngineError(
