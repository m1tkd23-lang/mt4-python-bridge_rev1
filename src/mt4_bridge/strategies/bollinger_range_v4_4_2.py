# src/mt4_bridge/strategies/bollinger_range_v4_4_2.py
...
# v4.4.2 追加:
# 弱いトレンド継続時に、バンド完全ブレイクでなくても
# 「ミドルより上/下で推移して再加速したら入る」補助条件
...
def _base_signal(
    ...
    if len(bars) < required_bars():
        raise SignalEngineError(
            f"At least {required_bars()} bars are required to evaluate bollinger_range_v4_4_2"
        )
...
def evaluate_bollinger_range_v4_4_2(
    market_snapshot: MarketSnapshot,
    position_snapshot: PositionSnapshot,
    strategy_name: str = "bollinger_range_v4_4_2",
) -> SignalDecision:
    bars = market_snapshot.bars
    if len(bars) < required_bars():
        raise SignalEngineError(
