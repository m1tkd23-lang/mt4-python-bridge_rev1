# src/mt4_bridge/strategies/bollinger_range_v4_4_3.py
...
# weak / strong trend 共通の continuation 補助エントリー
...
# weak trend 用 continuation エントリー
ENABLE_WEAK_TREND_CONTINUATION_ENTRY = True
WEAK_TREND_CONTINUATION_MIN_SLOPE = 0.00005
WEAK_TREND_CONTINUATION_PULLBACK_DISTANCE_THRESHOLD = 0.00060
...
def _weak_trend_continuation_buy_confirmed(
    *,
    ...
    return (
        trend_slope >= WEAK_TREND_CONTINUATION_MIN_SLOPE
        and trend_slope < TREND_SLOPE_THRESHOLD
        and previous_close >= middle_band
        and latest_close >= middle_band
        and latest_close >= trend_current_ma
        and latest_close > previous_close
        and latest_distance <= WEAK_TREND_CONTINUATION_PULLBACK_DISTANCE_THRESHOLD
        and latest_distance >= previous_distance
    )
...
def _base_signal(
    ...
    if len(bars) < required_bars():
        raise SignalEngineError(
            f"At least {required_bars()} bars are required to evaluate bollinger_range_v4_4_3"
        )
...
def evaluate_bollinger_range_v4_4_3(
    market_snapshot: MarketSnapshot,
    position_snapshot: PositionSnapshot,
    strategy_name: str = "bollinger_range_v4_4_3",
) -> SignalDecision:
    bars = market_snapshot.bars
    if len(bars) < required_bars():
        raise SignalEngineError(
    ...
    reason_suffix = (
        ...
        f" weak_trend_continuation_pullback_distance_threshold={WEAK_TREND_CONTINUATION_PULLBACK_DISTANCE_THRESHOLD},"
