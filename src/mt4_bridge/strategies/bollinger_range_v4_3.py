# src\mt4_bridge\strategies\bollinger_range_v4_3.py
...
# strong / weak trend のしきい値
STRONG_TREND_SLOPE_THRESHOLD = 0.0003
WEAK_TREND_SLOPE_THRESHOLD = 0.00015
...
# v4.3 では反対方向の weak/strong trend の両方でクローズ対象にする
CLOSE_ON_OPPOSITE_TREND_STATE = True

# =========================
# v4.2 由来: range failure exit
# =========================
...
def _determine_market_state(
    ...
    if _is_strong_trend_up(latest_close, trend_current_ma, trend_slope):
        return (
            "strong_trend_up",
            (
    ...
    if _is_strong_trend_down(latest_close, trend_current_ma, trend_slope):
        return (
            "strong_trend_down",
            (
    ...
    if _is_weak_trend_up(latest_close, trend_current_ma, trend_slope):
        return (
            "weak_trend_up",
            (
                f"weak_trend_up because trend_slope={trend_slope:.6f}"
                f" is between weak_threshold={WEAK_TREND_SLOPE_THRESHOLD:.6f}"
    ...
    if _is_weak_trend_down(latest_close, trend_current_ma, trend_slope):
        return (
            "weak_trend_down",
            (
...
def _base_signal(
    ...
    if len(bars) < required_bars():
        raise SignalEngineError(
            f"At least {required_bars()} bars are required to evaluate bollinger_range_v4_3"
        )
    ...
    latest_middle, latest_upper, latest_lower, latest_band_width = (
        _calculate_latest_bollinger_bands(closes)
    )
    ...
    if market_state == "range":
        ...
        return (
            SignalAction.HOLD,
            (
                f"range state but no confirmed reentry signal"
                f" (previous_close={previous_close}, latest_close={latest_close},"
                f" previous_upper={previous_upper}, latest_upper={latest_upper},"
                f" previous_lower={previous_lower}, latest_lower={latest_lower});"
                f" {state_reason}"
    ...
    if market_state == "strong_trend_up":
        if _trend_buy_confirmed(
            ...
            return (
                SignalAction.BUY,
                (
                    f"strong trend-follow buy confirmed by upper band breakout;"
                    f" previous_close={previous_close}, latest_close={latest_close},"
                    f" previous_upper={previous_upper}, latest_upper={latest_upper};"
                    f" {state_reason}"
                ),
        ...
        return (
            SignalAction.HOLD,
            (
                f"strong_trend_up state but no confirmed upper band breakout"
                f" (previous_close={previous_close}, latest_close={latest_close},"
                f" previous_upper={previous_upper}, latest_upper={latest_upper});"
                f" {state_reason}"
            ),
    ...
    if market_state == "strong_trend_down":
        if _trend_sell_confirmed(
            ...
            return (
                SignalAction.SELL,
                (
                    f"strong trend-follow sell confirmed by lower band breakout;"
                    f" previous_close={previous_close}, latest_close={latest_close},"
                    f" previous_lower={previous_lower}, latest_lower={latest_lower};"
                    f" {state_reason}"
                ),
        ...
        return (
            SignalAction.HOLD,
            (
                f"strong_trend_down state but no confirmed lower band breakout"
                f" (previous_close={previous_close}, latest_close={latest_close},"
                f" previous_lower={previous_lower}, latest_lower={latest_lower});"
                f" {state_reason}"
            ),
    ...
    if market_state == "weak_trend_up":
        return (
            SignalAction.HOLD,
            (
                f"weak_trend_up state so entry is skipped"
                f" (previous_close={previous_close}, latest_close={latest_close},"
                f" previous_upper={previous_upper}, latest_upper={latest_upper});"
                f" {state_reason}"
            ),
    ...
    if market_state == "weak_trend_down":
        return (
            SignalAction.HOLD,
            (
                f"weak_trend_down state so entry is skipped"
                f" (previous_close={previous_close}, latest_close={latest_close},"
                f" previous_lower={previous_lower}, latest_lower={latest_lower});"
                f" {state_reason}"
            ),
...
def evaluate_bollinger_range_v4_3(
    market_snapshot: MarketSnapshot,
    position_snapshot: PositionSnapshot,
    strategy_name: str = "bollinger_range_v4_3",
) -> SignalDecision:
    bars = market_snapshot.bars
    if len(bars) < required_bars():
        raise SignalEngineError(
    ...
    if market_state == "range" and EXIT_ON_RANGE_MIDDLE_BAND:
        if current_type == "buy" and latest_bar.high >= middle_band:
            return _build_signal_decision(
                action=SignalAction.CLOSE,
                reason=(
        ...
        if current_type == "sell" and latest_bar.low <= middle_band:
            return _build_signal_decision(
                action=SignalAction.CLOSE,
                reason=(
    ...
    if CLOSE_ON_OPPOSITE_TREND_STATE:
        if current_type == "buy" and market_state in ("weak_trend_down", "strong_trend_down"):
            return _build_signal_decision(
                action=SignalAction.CLOSE,
                reason=(
                    f"buy position closed because state switched to {market_state}"
                    + reason_suffix
                ),
                **common_kwargs,
            )

        if current_type == "sell" and market_state in ("weak_trend_up", "strong_trend_up"):
            return _build_signal_decision(
                action=SignalAction.CLOSE,
                reason=(
                    f"sell position closed because state switched to {market_state}"
                    + reason_suffix
                ),
                **common_kwargs,
            )
