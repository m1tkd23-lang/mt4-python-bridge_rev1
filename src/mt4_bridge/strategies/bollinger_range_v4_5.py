# src/mt4_bridge/strategies/bollinger_range_v4_5.py
...
BOLLINGER_PERIOD = 20
BOLLINGER_SIGMA = 2.0
BOLLINGER_EXTREME_SIGMA = 3.0  # 3σタッチ用
...
# エントリー確認
# range:
#   previous_close が 2σ バンド外
#   latest_close が 2σ バンド内へ戻ったらエントリー
RANGE_REQUIRE_REENTRY_CONFIRMATION = True

# trend:
#   previous_close がまだ 2σ ブレイクしておらず
#   latest_close で 2σ ブレイクしたらエントリー
...
# =========================
# v4.4 / v4.5: 3σタッチ即時エントリー
# =========================
# 足確定時に high/low が 3σ にタッチしていれば即エントリー
ENABLE_RANGE_EXTREME_TOUCH_ENTRY = True

# v4.5 追加:
# trend_up / trend_down 判定の足では 3σ エントリーを禁止する
...
def _base_signal(
    ...
    if len(bars) < required_bars():
        raise SignalEngineError(
            f"At least {required_bars()} bars are required to evaluate bollinger_range_v4_5"
        )
    ...
    extreme_touch_allowed = ENABLE_RANGE_EXTREME_TOUCH_ENTRY and not (
        DISABLE_EXTREME_TOUCH_ENTRY_IN_TREND
        and market_state in {"trend_up", "trend_down"}
    )

    if market_state == "range":
        if extreme_touch_allowed:
        ...
        no_signal_reason = (
            ...
            f" extreme_touch_allowed={extreme_touch_allowed});"
            f" {state_reason}"
        )
        return (
            SignalAction.HOLD,
...
def evaluate_bollinger_range_v4_5(
    market_snapshot: MarketSnapshot,
    position_snapshot: PositionSnapshot,
    strategy_name: str = "bollinger_range_v4_5",
) -> SignalDecision:
    bars = market_snapshot.bars
    if len(bars) < required_bars():
        raise SignalEngineError(
