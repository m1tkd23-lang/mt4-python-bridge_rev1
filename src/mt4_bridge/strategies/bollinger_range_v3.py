# src\mt4_bridge\strategies\bollinger_range_v3.py
...
def _determine_market_state(
    ...
    if _is_range(range_slope, normalized_band_width):
        return (
            "range",
            (
...
def _base_signal(
    ...
    if len(bars) < required_bars():
        raise SignalEngineError(
            f"At least {required_bars()} bars are required to evaluate bollinger_range_v3"
        )
    ...
    normalized_width = _normalized_band_width(middle_band, band_width)
...
def evaluate_bollinger_range_v3(
    market_snapshot: MarketSnapshot,
    position_snapshot: PositionSnapshot,
    strategy_name: str = "bollinger_range_v3",
) -> SignalDecision:
    bars = market_snapshot.bars
    if len(bars) < required_bars():
        raise SignalEngineError(
    ...
    reason_suffix = (
        ...
        f" lower={lower_band}, normalized_band_width={normalized_width:.6f},"
        f" range_slope={range_slope:.6f}, trend_slope={trend_slope:.6f},"
