# src\mt4_bridge\strategies\bollinger_range_v6_1.py
...
# v6 では切りすぎたため無効化
RANGE_BAND_RETEST_EXIT_ENABLED = False
RANGE_BAND_RETEST_BUFFER_RATIO = 0.12

# v6 では切りすぎたため無効化
TREND_MA_EXIT_ENABLED = False
...
def _base_signal(
    ...
    if len(bars) < required_bars():
        raise SignalEngineError(
            f"At least {required_bars()} bars are required to evaluate bollinger_range_v6_1"
        )
...
def evaluate_bollinger_range_v6_1(
    market_snapshot: MarketSnapshot,
    position_snapshot: PositionSnapshot,
    strategy_name: str = "bollinger_range_v6_1",
) -> SignalDecision:
    bars = market_snapshot.bars
    if len(bars) < required_bars():
        raise SignalEngineError(
