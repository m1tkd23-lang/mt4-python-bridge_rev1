# src/mt4_bridge/strategies/bollinger_range_v4_4_guarded_bac.py
...
# 絶対傾き。
# USDJPY M5 前提では小さすぎると効きすぎるため、
# まずは控えめな初期値にしてログで調整する前提。
...
def _compute_band_snapshot(closes: list[float]) -> dict[str, float]:
    if len(closes) != 20:
        raise SignalEngineError("Band snapshot requires exactly 20 closes")
...
def _detect_risk_flags(market_snapshot: MarketSnapshot) -> dict[str, bool | float | int]:
    ...
    latest_window = closes[-20:]
    prev_window = closes[-21:-1]
    prev2_window = closes[-22:-2]
...
def evaluate_bollinger_range_v4_4_guarded_bac(
    market_snapshot: MarketSnapshot,
    position_snapshot: PositionSnapshot,
    strategy_name: str = "bollinger_range_v4_4_guarded_bac",
) -> SignalDecision:
