...
def _base_signal(
    ...
    if len(bars) < required_bars():
        raise SignalEngineError(
            f"At least {required_bars()} bars are required to evaluate example_strategy_v1"
        )
    ...
    if latest_bar.close > previous_bar.close:
        return SignalAction.BUY, "latest close is greater than previous close"

    if latest_bar.close < previous_bar.close:
        return SignalAction.SELL, "latest close is lower than previous close"

    return SignalAction.HOLD, "latest close is equal to previous close"


def evaluate_example_strategy_v1(
    market_snapshot: MarketSnapshot,
    position_snapshot: PositionSnapshot,
    strategy_name: str = "example_strategy_v1",
) -> SignalDecision:
    bars = market_snapshot.bars
    if len(bars) < required_bars():
        raise SignalEngineError(
