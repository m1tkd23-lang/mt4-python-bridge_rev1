# src\mt4_bridge\strategies\bollinger_range_v4.py
...
def _base_signal(
    ...
    if len(bars) < required_bars():
        raise SignalEngineError(
            f"At least {required_bars()} bars are required to evaluate bollinger_range_v4"
        )
    ...
    previous_middle, previous_upper, previous_lower, previous_band_width = _calculate_previous_bollinger_bands(closes)
...
def evaluate_bollinger_range_v4(
    market_snapshot: MarketSnapshot,
    position_snapshot: PositionSnapshot,
    strategy_name: str = "bollinger_range_v4",
) -> SignalDecision:
    bars = market_snapshot.bars
    if len(bars) < required_bars():
        raise SignalEngineError(
