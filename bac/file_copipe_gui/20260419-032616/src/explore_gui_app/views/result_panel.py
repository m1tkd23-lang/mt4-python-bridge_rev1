...
class ExploreResultPanel(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._phase_label.setStyleSheet("font-size: 13px; font-weight: bold; padding: 4px;")
        ...
        for i, (key, label_text) in enumerate(summary_fields):
            if key == "assessment":
                # Assessment gets its own full-width row
                lbl = QLabel(f"{label_text}: -")
                lbl.setStyleSheet("font-size: 13px; font-weight: bold; padding: 2px;")
    ...
    def add_iteration_result(
        ...
        row_color = self._verdict_color(result.verdict)
    ...
    def show_monthly_breakdown(self, result: BollingerExplorationResult) -> None:
        ...
        for entry in entries:
            ...
            self._monthly_table.setItem(row, 0, QTableWidgetItem(entry.label))
    ...
    def add_phase2_result(
        ...
        pf = f"{agg.overall_profit_factor:.2f}" if agg and agg.overall_profit_factor is not None else "-"
    ...
    def show_phase2_summary(
        ...
        # Pick the best result by average_pips_per_month
        best = max(
            results,
            key=lambda r: (
                r.aggregate_stats.average_pips_per_month
                if r.aggregate_stats else 0.0
            ),
        )
        ...
        self._summary_labels["total_pips"].setText(
            f"Total Pips: {agg.total_pips:.1f}"
        )
        self._summary_labels["avg_pips"].setText(
            f"Avg Pips/Month: {agg.average_pips_per_month:.1f}"
        )
        pf_str = f"{agg.overall_profit_factor:.2f}" if agg.overall_profit_factor is not None else "-"
        ...
        stddev_str = f"{agg.monthly_pips_stddev:.1f}" if agg.monthly_pips_stddev is not None else "-"
        self._summary_labels["stddev"].setText(
            f"Monthly Pips StdDev: {stddev_str}"
        )
        self._summary_labels["deficit_months"].setText(
            f"Deficit Months: {agg.deficit_month_count} / {agg.month_count}"
        )
        mfe_str = f"{agg.avg_mfe_mae_ratio:.2f}" if agg.avg_mfe_mae_ratio is not None else "-"
        self._summary_labels["mfe_mae"].setText(
            f"Avg MFE/MAE Ratio: {mfe_str}"
        )

        # Simple adoption assessment heuristic
        issues: list[str] = []
        ...
        self._summary_labels["assessment"].setText(
    ...
    def _verdict_color(verdict: str) -> QColor | None:
        """Return a background color for a table row based on verdict."""
        v = (verdict or "").lower()
        if v == "adopt":
            return QColor(232, 245, 233)  # light green
        if v == "improve":
            return QColor(255, 243, 224)  # light orange
        if v == "discard":
